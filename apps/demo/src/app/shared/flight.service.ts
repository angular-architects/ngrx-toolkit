...
export class FlightService implements DataService<Flight, FlightFilter> {
  ...
  update(entity: Partial<Flight>): Promise<Flight> {
    return firstValueFrom(this.save(entity));
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  updateAll(entity: Array<Partial<Flight>[]>): Promise<Flight[]> {
  ...
  private save(flight: Flight | Partial<Flight>): Observable<Flight> {
    const url = [this.baseUrl, 'flight'].join('/');
